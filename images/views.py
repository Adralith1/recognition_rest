...
class ImgSearch(APIView):
    ...
    def post(self, request, format=None, **kwargs): #POST de l'image en b64
        ...
        request.data['base64']="" #J'efface l'image du client en base64 par soucis de poids dans la base de donnees (mais on peut choisir de la garder si il veut la pargager
        if serializer.is_valid():
            serializer.save() #on ecrit dans la bd les datas du serializer
            conn = sqlite3.connect("db.sqlite3") #connexion a la bd
            cursor = conn.cursor()
            cursor.execute('SELECT id FROM images_request WHERE request_date=?', (date_format,))#recherche du dernier id cree
            ...
            if "png" in base_64PourAnalyse: #si le type de l'image c'est png
                ...
                typeimg = "jpg" #ou jpg
            img_construite = test.buildAndReturnLabel(base_64PourAnalyse,typeimg)  # cette fonction nous retourne l'image construite et la compare avec les autres
            ...
            response = Response({"Location" : "/img_searches/"+ str(id_cree[0][0])} , status=status.HTTP_201_CREATED) #retourne une location avec l'ID de l'objet cree en BD
            return response
